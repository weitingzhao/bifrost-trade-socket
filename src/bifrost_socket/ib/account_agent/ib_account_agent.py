...
_SERVICE_HEARTBEAT_CONNECT_TIMEOUT_SEC = 5.0


async def _heartbeat_reconnect_slot(client: AccountIbClient, *, label: str) -> None:
    """Single service-heartbeat reconnect for one IB client slot; errors stay local."""
    try:
        await asyncio.wait_for(
            client.ensure_connected(max_connect_attempts=1),
            timeout=_SERVICE_HEARTBEAT_CONNECT_TIMEOUT_SEC,
        )
    except asyncio.TimeoutError:
        logger.debug("account agent heartbeat %s: connect timed out after %.0fs", label, _SERVICE_HEARTBEAT_CONNECT_TIMEOUT_SEC)
    except Exception as e:
        logger.debug("account agent heartbeat %s: %s", label, e)
...
class IbAccountAgent:
    ...
    async def _probe_loop(
        ...
        while not self._stop.is_set():
            ...
            # Service heartbeat reconnect — parallel for Host + Secondary (intentional).
            hb_tasks: List[Awaitable[None]] = []
            if not host_ok:
                hb_tasks.append(_heartbeat_reconnect_slot(primary, label="Host"))
            if secondary is not None and not sec_ok:
                hb_tasks.append(_heartbeat_reconnect_slot(secondary, label="Secondary"))
            if hb_tasks:
